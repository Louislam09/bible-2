export default [
  {
    id: '02',
    title: '02 - ¡YO TENGO UN HIMNO DE LOOR!',
    chorus:
      '//Desde que salvo estoy: //\nSólo en El me gloriaré,\nDesde que salvo estoy,\nEn mi Salvador me gloriaré.',
    stanzas: [
      '(1)Yo tengo un himno de loor,\nDesde que salvo estoy,\nPara mi Rey, mi Salvador,\nDesde que salvo estoy.',
      '(2)Yo tengo un Cristo, y mi ansiedad\nDesde que salvo estoy,\nEstá en cumplir su voluntad,\nDesde que salvo estoy.',
      '(3)Yo tengo un gozo que El me dio\nDesde que salvo estoy,\nCuando en su sangre me lavó,\nDesde que salvo estoy.',
      '(4)Tengo un hogar a donde iré,\nDesde que salvo estoy,\nY allí seguro viviré\nDesde que salvo estoy.',
    ],
  },
  {
    id: '07',
    title: '07 - ¡DE MI TIERNO SALVADOR!',
    chorus:
      '¡Mi Jesús! ¡Mi Jesús!\n¡Cuán precioso es el nombre de Jesús!\nCon su sangre me limpió,\nDe su gozo me llenó,\nDe su vida me dotó, mi Jesús.',
    stanzas: [
      '(1)De mi tierno Salvador\nCantaré el inmenso amor,\nGloriaréme en el favor de Jesús:\nDe tinieblas me llamó,\nDe cadenas me libró,\nDe la muerte me salvó, mi Jesús.',
      '(2)¡Oh, qué triste condición\nDel impío corazón!\nMe salvó de perdición, mi Jesús\nDel pecado, el perdón,\nDe la ruina, salvación:\nPor tristeza, bendición, dio Jesús.',
      '(3)En el mundo al vagar,\nSolitario sin hogar,\nNo sabía que dulce paz da Jesús;\nMas las lágrimas de ayer\nHan pasado, y placer\nYa comienzo a tener en Jesús',
      '(4)De lo falso a su verdad\nDe lo inmundo a santidad,\nYa me trajo la bondad de Jesús.\nHechos fuertes en virtud\nDe su perennal salud;\nHimnos dad de gratitud a Jesús.',
    ],
  },
  {
    id: '08',
    title: '08 - ¡HOSANNA!',
    chorus: null,
    stanzas: [
      '¡Hosanna! ¡Hosanna! ¡Hosanna!\nEn cielo y tierra es del Señor\nLa gloria y potestad;\nY nos circunda con su amor\nLa excelsa divinidad.\nAlzad pues himnos de loor.\nQue es grato al sumo bien,\nY a Dios rindamos todo honor\nAhora y siempre. Amén.\nA Dios rindamos todo honor,\nTodo honor, Todo honor,\nA Dios rindamos todo honor\nAhora y siempre Amén… Amén.',
    ],
  },
  {
    id: '09',
    title: '09 - ¡SANTO! ¡SANTO! ¡SANTO!',
    chorus: null,
    stanzas: [
      '(1)¡Santo! ¡Santo! ¡Santo! Señor omnipotente\nSiempre el labio mio loores te dará;\n¡Santo! ¡Santo! ¡Santo! Te adoro reverente\nDios Padre y su Hijo, y su Santo Espíritu.',
      '(2)¡Santo! ¡Santo! ¡Santo! En numeroso coro,\nSantos escogidos te adoran con fervor;\nDe alegría llenos, y sus coronas de oro,\nRinden ante el trono glorioso del Señor.',
      '(3)¡Santo! ¡Santo! ¡Santo! la Inmensa muchedumbre\nDe ángeles que cumplen tu santa voluntad\nAnte ti se postra bañada con tu lumbre;\nAnte ti que has sido, que eres y serás.',
      '(4)¡Santo! ¡Santo! ¡Santo! por más que estés velado,\nE imposible sea tu gloria contemplar.\nSanto tu eres sólo y nada hay a tu lado,\nEn poder perfecto, pureza y caridad.',
      '(5)¡Santo! ¡Santo! ¡Santo! La gloria de tu nombre\nVemos en tus obras, en cielo, tierra y mar;\n¡Santo! ¡Santo! ¡Santo! Te adorará todo hombre:\nDios Padre y su Hijo, y su Santo Espíritu.',
    ],
  },
  {
    id: '10',
    title: '10 - ¡TAN DULCE EL NOMBRE DE JESUS!',
    chorus:
      '¡Cristo, oh qué dulce es;\nCristo, para siempre es;\nCristo, yo te aclamaré,\nPor siempre, ¡Oh mi Cristo!',
    stanzas: [
      '(1)¡Tan dulce el nombre de Jesús!\nSus bellas notas al cantar,\nQue mi alma llena al proclamar,\n¡El nombre de Jesús!',
      '(2)Adoro el nombre de Jesús;\nJamás me faltará su amor;\nY pone aparte mi dolor,\n¡El nombre de Jesús!',
      '(3)Tan puro el nombre de Jesús\nQue mi pesar pudo quitar,\nY grata paz a mi alma dar,\n¡El nombre de Jesús!',
      '(4)El dulce nombre de Jesús,\nPor siempre quiero alabar;\nY todos deben exaltar\nEl nombre de Jesús.',
    ],
  },
  {
    id: '13',
    title: '13 - ¡LA GLORIA DE CRISTO!',
    chorus:
      'Es todo bondad para mí,\nCon Él nada puedo desear;\nPues todos mis altos deseos aquí,\nTan sólo Él los puede llenar.',
    stanzas: [
      '(1)La gloria de Cristo el Señor cantaré,\nPues llena mi vida de gozo y de paz;\nCallar los favores que de Él alcancé,\nMis labios no pueden jamás.',
      '(2)En horas de angustia conmigo Él está\nY puedo escuchar su dulcísima voz,\nQue me habla, y su paz inefable me da,\nLa paz infinita de Dios',
      '(3)Si a rudos conflictos me mira que voy,\nMe deja hasta el fin a mi solo luchar:\nMás pronto si ve que cediendo ya estoy,\nSocorro me viene a prestar.',
      '(4)También cuando gozo lo miro llegar,\nY entonces mi dicha la aumenta el Señor\nYa llena mi copa la veo rebosar,\nCon todos sus dones de amor.',
    ],
  },
  {
    id: '14',
    title: '14 - ¡VED AL CRISTO!',
    chorus:
      'Coronadle, santos todos,\nCoronad al Rey de reyes;\nCoronadle, santos todos\nCoronad al Salvador.',
    stanzas: [
      '(1)Ved al Cristo, Rey de Gloria\nEs del mundo el vencedor;\nDe la guerra vuelve invicto,\nTodos démosle loor.',
      '(2)Exaltado sí, exaltado\nRicos triunfos trae Jesús\nEntronadle allá en los cielos\nEn la refulgente luz.',
      '(3)Si los malos se burlaron\nCoronando al Salvador,\nHoy los ángeles y santos,\nLo proclaman su Señor.',
      '(4)Escuchad sus alabanzas,\nQue se elevan hacia Él;\nVictorioso reina el Cristo,\n¡Adorad a Emmanuel!',
    ],
  },
  {
    id: '16',
    title: '16 - ¡LA MAÑANA VINO YA!',
    chorus:
      'La mañana reina ya en mi corazón,\nToda sombra ha disipado mi Señor,\nYa cantando alegre voy,\nPorque con Jesús estoy\nLa mañana reina ya en mi corazón.',
    stanzas: [
      '(1)Ya la noche con sus sombras se alejó,\nLa mañana vino ya;\nY viviendo en plena luz me encuentro yo,\nLa mañana vino ya.',
      '(2)Hace Cristo un mundo nuevo para mí,\nLa mañana vino ya;\nY por Él y en Él la vida es grande aquí\nLa mañana vino ya.',
      '(3)Una vida esplendorosa viviré,\nLa mañana vino ya;\nPorque en Cristo mi Señor me apoyaré,\nLa mañana vino ya;',
    ],
  },
  {
    id: '17',
    title: '17 - ¡CUAN GRANDE ES EL!',
    chorus:
      'Mi corazón entona la canción\n¡Cuán grande es Él! ¡Cuán grande es Él!\nMi corazón entona la canción\n¡Cuán grande es Él! ¡Cuán grande es Dios!',
    stanzas: [
      '(1)Señor mi Dios, al contemplar los cielos,\nEl firmamento y las estrellas mil;\nAl oír tu voz en los potentes truenos\nY ver brillar al sol en su cenit.',
      '(2)Al recorrer los montes y los valles\nY ver las bellas flores al pasar;\nAl escuchar el canto de las aves\nY el murmullar del claro manantial.',
      '(3)Cuando recuerdo del amor divino,\nQue desde el cielo al salvador envió;\nAl buen Jesús que por salvarme vino;\nY en una cruz sufrió y por mí murió.',
      '(4)Cuando el Señor me llame a su presencia,\nAl dulce hogar, al cielo de esplendor;\nLe adoraré cantando la grandeza\nDe su poder y su infinito amor.',
    ],
  },
  {
    id: '21',
    title: '21 - ¡HAY UN CANTO NUEVO!',
    chorus:
      'Cristo, Cristo, Cristo,\nNombre sin igual;\nLlena siempre mi alma,\nDe esa nota celestial.',
    stanzas: [
      '(1)Hay un canto nuevo en mi ser,\nEs la voz de mi Jesús,\nQue me dice: "Ven a descansar,\nTu paz conquisté en la cruz".',
      '(2)Náufrago en pecado me encontré,\nSin paz en mi corazón;\nMas en Cristo mi Señor hallé,\nDulce paz y protección.',
      '(3)Tengo de su gracia celestial,\nBajo sus alas de amor,\nRiquezas que fluyen a raudal,\nDesde el trono del Señor.',
      '(4)Por las aguas hondas me llevó,\nPruebas en mi senda hallé,\nPor sendero áspero me guió;\nMas sus hullas seguiré.',
      '(5)Cristo en las nubes volverá,\nBajo el bello cielo azul,\nEl entonces mi alma llevará,\nA vivir en gloria y luz.',
    ],
  },
  {
    id: '27',
    title: '27 - ¡CANTARE A JESUS!',
    chorus:
      'Yo soy feliz con mi Cristo, Porque Él mi alma salvó.\nÁngeles cantan alegres, Por cuantos El redimió.\n¡Oh! Cuan hermosa es la vida, Dulce, serena de fe\nSoy peregrino en la tierra,\nMás ando con Cristo, de mi alma Rey.',
    stanzas: [
      '(1)Cantaré a Jesús, de mi alma Rey,\nAl Rey de gloria exaltaré;\nTrajo al mundo paz, y al mortal buena voluntad,\nAlegres nuevas doquiera proclamad.',
      '(2)En la cruz del calvario por mí sufrió,\nAsí la amarga copa bebió;\nY al expirar, el sol de luto se cubrió,\nMás vencedor el Señor resucitó.',
      '(3)Ascendió a la gloria mi Salvador,\nPor mí intercede con tierno amor;\nPronto volverá con sus santos para reinar;\n¡Dicha indecible será con El morar!',
      '(4)¿Qué será en aquella mansión de luz?\nAl ver el rostro de mi Jesús,\nY con El gozar las grandezas que allí habrá,\nY los placeres que el fiel disfrutará.',
    ],
  },
  {
    id: '29',
    title: '29 - ¡LOOR A TI MI DIOS!',
    chorus: null,
    stanzas: [
      '(1)Loor a ti, mi Dios, en esta noche\nPor todas tus bondades de este día,\nOh guárdame, y que tus potentes alas\nSean por siempre la defensa mía.',
      '(2)De cuantas faltas hoy eh cometido,\nPerdóname, Señor por tu Hijo amado;\nContigo, con el prójimo y conmigo\nQuede antes de dormir reconciliado.',
      '(3)Enséñame a vivir, que no me espante\nLa tumba más que el lecho de reposo.\nEnséñame a morir, para que pueda,\nEl día del juicio, despertar glorioso.',
      '(4)¡Oh! Logre reposar en ti mi alma,\nMis párpados los cierre dulce sueño,\nUn sueño que vigor pueda prestarme\nPara servirte, al despertar, mi Dueño',
    ],
  },
  {
    id: '33',
    title: '33 - ¡TIERNAS CANCIONES!',
    chorus:
      '¡Cielo y tierra canten Al Señor de las naciones,\nCielo y tierra canten Al Señor de las naciones,\nY los hombres todos, Con alegres corazones.\nSirvan al Señor, Que vida y paz siempre les da!',
    stanzas: [
      '(1)Tiernas canciones alzad al Señor,\nHimnos que lleven del alma la fe,\nY hablen muy alto del férvido amor,\nQue hay en el pecho del hombre que cree\nVengan trayendo ferviente canción\nNiños y ancianos de Dios al altar.\nTraigan a El su corazón.\nÚnico don que podrá aceptar.',
      '(2)El es la fuente de toda bondad,\nEs de la vida, la luz y el calor,\nSólo El nos libra de cruel ansiedad,\nSólo El aleja del alma el dolor;\nDigno es, por tanto, que el hombre le de\nGloria y honor que resuenen doquier,\nVamos a El, llenos de fe,\nNos salvará con su gran poder.',
    ],
  },
  {
    id: '38',
    title: '38 - ¡BENDICE OH ALMA!',
    chorus:
      '¡Cantad, cantad, dulcísima canción,\nCantad, cantad, al Rey de la creación;\nVosotros, ángeles que El creó,\nY hacéis su voluntad,\nVosotras, obras que El formó,\nSu nombre celebrar.',
    stanzas: [
      '(1)Bendice oh alma; a tu creador\nCon férvida canción,\nDespierta y cante su loor,\nTu ardiente corazón.\nNo olvides nunca su bondad,\nCon negra ingratitud\nPues El perdona tu maldad,\nTe salva y da salud.',
      '(2)De bien tu boca llena está\nY nueva juventud\nA tu alma siempre le dará,\nCalmando tu inquietud,\nEn ira es lento para arder\nMás pronto es en su amor,\nNo quiere siempre contender\nNi dura su furor.',
      '(3)Tan alto cual el cielo creó\nSobre este mundo cruel,\nAsí su amor engrandeció,\nAl darlo al hombre fiel,\nY tan lejano cual están\nOeste y este, así\nMis rebeliones estarán\nMuy lejos, sí de mí.',
      '(4)Se acuerda que del polvo soy\nFormado en su poder,\nQue el hombre es flor tan sólo de hoy\nQue al sol va perecer\nMás El de siglo en siglo da\nSu amor sin limitar,\nA todo aquél que listo está\nSus leyes a guardar.',
    ],
  },
  {
    id: '45',
    title: '45 - ¡BELLAS PALABRAS DE VIDA!',
    chorus: null,
    stanzas: [
      '(1)¡Oh cantádmelas otra vez!\nBellas palabras de vida;\nHallo en ellas mi gozo y luz,\nBellas palabras de vida;\nSí de luz y vida;\nSon sostén y guía:\n::!Que bellas son, que bellas son!\nBellas palabras de vida!::',
      '(2)Jesucristo a todos da\nBellas palabras de vida;\nHoy escúchalas pecador;\nBellas palabras de vida;\nBondadoso te salva.\nY al cielo te llama:\n::!Que bellas son, que bellas son!\nBellas palabras de vida!::',
      '(3)Grato el cántico sonará,\nBellas palabras de vida;\nTus pecados perdonará,\nBellas palabras de vida;\nSí de luz y vida;\nSon sostén y guía:\n!Que bellas son, que bellas son!\nBellas palabras de vida!::',
    ],
  },
  {
    id: '48',
    title: '48 - ¡ANHELO YO OIR LA VOZ!',
    chorus:
      '¡Tus palabras, oh Jesús!\n¡Tus palaabras vida son!\nMe enseñan, alumbran,\nMe traen la luz;\nLas palabras de Jesús.',
    stanzas: [
      '(1)Anhelo yo oír la voz\nDe mi buen Salvador\nSus dichos un banquete son,\nMi verdadera luz;\nComo el maná, a mi alma dan\nDulzura y sostén\nConsuelo, paz, recibo aquí\nDel maestro celestial.',
      '(2)Si las palabras de Jesús\nInfunden fuerza y fe;\nAlejan de mi corazón,\nLas dudas, yo lo sé;\nLa mansedumbre es de El\nCon El aprenderé\nY en la gloria su amor,\nPor siempre alabaré.',
      '(3)En todo tiempo mi Jesús,\nPalabras me tendrá;\nEn pruebas ellas son mi bien;\nSu voz me alentará;\nEn ellas mi alma tiene paz,\n¿Quién la podrá turbar?\nJesús me ofrece un festín,\nNo más podré desear.',
      '(4)Lugar no hay, no lo habrá,\nEn este mundo, no,\nEn que el alma goce paz,\nSi esto olvido;\nPues su palabra fuente es,\nDe toda bendición,\nY medio sólo del Señor\nPara la salvación.',
    ],
  },
  {
    id: '49',
    title: '49 - ¡LIBRO SANTO!',
    chorus: null,
    stanzas: [
      '(1) Libro Santo por Dios inspirado,\nManantial de verdades eternas,\nHas venido a romper las cadenas\nDel pecado, ignorancia y temor,\nTú nos hablas de regias mansiones,\nDe un destino feliz más allá,\nManifiesto a las vista nos pones,\nLo que al santo su Dios le dará.',
      '(2) Es perfecto el mandato divino\nRectamente encamina a las almas.\nAl pequeño a quien tú tanto amás,\nTus preceptos lo hacen crecer.\nPaz y gozo perfecto le has dado,\nAl mortal que en tus aguas bebió.\nGalardón inmortal ha logrado\nEl que fiel tu mandato guardó.',
      '(3) Sigue siempre brillando en el mundo,\nQue millones tu luz desconocen.\nY a fin que tu influencia ellos gocen,\nEn el mundo te haré circular.\n¡Dios bendito, despeja mi mente\nPara ver tu perfecta verdad\nHaz la guarde por siempre obediente\nAlabando tu excelsa bondad!',
    ],
  },
  {
    id: '51',
    title: '51 - DEL SANTO AMOR DE CRISTO',
    chorus:
      'El amor de mi Señor, grande y dulce es más y más\nRico e inefable, nada es comparable,\nAl amor de mi Jesús.',
    stanzas: [
      '(1)Del santo amor de Cristo que no tendrá su igual;\nDe su divina gracia, sublime y eternal;\nDe su misericordia, inmensa como el mar\nY cual los cielos alta, con gozo eh de cantar.',
      '(2)Cuando El vivió en el mundo la gente lo siguió,\nY todas sus angustias en El deposito:\nEntonces, bondadoso, su amor brotó en raudal,\nIncontenible, inmenso, sanando todo mal.',
      '(3)El puso en las pupilas del ciego nueva luz;\nLa eterna luz de vida que centella en la cruz;\nY dio a las almas todas la gloria de su Ser,\nAl impartir su gracia, su espíritu y poder.',
      '(4)Su amor, por las edades del mundo es el fanal,\nQue marca esplendoroso la senda ideal;\nY al paso de los años, lo hará más dulce y más,\nPreciso al darle al alma, su incomparable paz.',
    ],
  },
  {
    id: '54',
    title: '54 - OH BONDAD TAN INFINITA',
    chorus:
      'Es Jesús para mí,\nLa esperanza de salud;\nSólo en El hallaré,\nLa divina plenitud.',
    stanzas: [
      '(1)Oh bondad tan infinita,\nHacia el mundo pecador,\nDios en Cristo revelando\nSu eternal y santo amor.',
      '(2)Como el vasto firmamento,\nComo el insondable mar,\nEs la gracia salvadora\nQue Jesús al alma da.',
      '(3)Aunque fueren tus pecados,\nRojos como el carmesí,\nen el río del Calvario,\nHay limpieza para ti.',
    ],
  },
  {
    id: '58',
    title: '58 - LEJOS DE MI DULCE HOGAR',
    chorus: ':: !Su grande amor me levantó\nDe densa oscuridad me libertó!::',
    stanzas: [
      '(1)Lejos de mi dulce hogar, vagaba yo sin Dios,\nA través de tierra y mar, sin esperanza y paz;\nMas el tierno salvador, viéndome en aflicción.\nPor su infinito amor me levantó.',
      '(2)Todo entrego a mi Jesús, siempre le seguiré.\nHe tomado ya la cruz y el mundo atrás dejé,\nTan excelso y grande amor requiere la canción,\nY el servicio fiel de cada corazón.',
      '(3)Ven a El, ¡oh! Pecador, no te rechazará:\nCon ternura el buen Pastor hoy te recibirá,\nTus pecados borrará gozo tendrás sin par,\nGracia y fuerza te dará para triunfar',
    ],
  },
  {
    id: '59',
    title: '59 - EN UNA CRUZ',
    chorus:
      '¡Divino amor! ¡Divino amor!\nTu gran poder me conquistó.\nSerá de ti mi corazón,\n¡Divino amor! ¡Divino amor!',
    stanzas: [
      '(1)En una cruz de cruel dolor\nPendiente a Cristo contemplé.\nPerdido entonces me miré,\n¡Divino amor! ¡Divino amor!',
      '(2)Tú me miraste, buen Señor,\nArdiendo en santa compasión,\nTu acento fue de salvación\n¡Divino amor! ¡Divino amor!',
      '(3)Pródigo anduve en mal y error,\nDel dulce hogar errante fui,\nTu eterno amor fue en pos de mí.\n¡Divino amor! ¡Divino amor!',
      '(4)No más rebelde, mi Señor\nNi vil esclavo habré de ser.\nProtéjame tu gran poder.\n¡Divino amor! ¡Divino amor!',
    ],
  },
  {
    id: '64',
    title: '64 - HABLA A MI OH CRISTO',
    chorus:
      '¡Habla a mí, oh Cristo, habla en gran amor!\n¡Tómame en tus brazos, llena mi corazón!\nEstoy postrado a tus pies, escuchando tu voz.\nDame descanso, gozo, paz y tranquilidad.',
    stanzas: [
      '(1)¡Habla a mí, oh Cristo!; habla yo quiero oír!\n¡Muéstrame tu camino! Con gozo eh de seguir.\nCuando estaba triste, cansado vine a ti.\nTú mi descanso diste, paz y tranquilidad.',
      '(2)Mil voces, sí me llaman: ¡guárdame fiel Señor!\nQue yo en este mundo nunca te pierda a ti.\nSólo tú das descanso, sólo tú das perdón.\nTu carga es ligera, tu yugo fácil es.',
      '(3)Confiando en los hombres, sufrí engaño vil,\nAhora en ti confío, siempre tú eres fiel.\nPara ser victorioso en este mundo aquí,\nYo debo, ¡oh mi Cristo! Sólo confiar en ti.',
    ],
    // second_chorus:
    //   'Hablas a mí, ¡oh, Cristo! Hablas en gran amor.\nMe llevas en tus brazos, llenas mi corazón,\nTu gran poder conozco; victoria tengo en ti.\nTu plenitud hermosa disfruto yo aquí.',
  },
  {
    id: '65',
    title: '65 - HERMOSO AMOR',
    chorus:
      'Su grande amor a mi me salvó.\nSu grande amor me libertó,\nEn una cruz mi Jesús sufrió\nPor ese amor que tiene El para mí',
    stanzas: [
      '(1)¡Hermoso amor que me salvó de perdición!\nCulpable y vil pecador, yo iba a morir.\nSin esperanza de vivir, ¡oh día feliz!\nCuando Jesús me levantó por su amor.',
      '(2)En una cruz mi salvador vino a morir,\nPor un malvado como yo, su vida dio,\nPor ese amor que en mí mostró: ¡Gloria al Señor!\nSoy un cautivo en libertad por su amor.',
      '(3)Jesús mi salvador mi Rey me prometió\nUna mansión donde estaré siempre con El.\nY allí tristezas o dolo jamás tendré\nEsa mansión me preparó por su amor.',
    ],
  },
  {
    id: '69',
    title: '69 - ME GUIA EL',
    chorus:
      'Me guía El, me guía El\nCon cuánto amor me guía El;\nNo abrigo dudas ni temor,\nPues me conduce el Buen Pastor.',
    stanzas: [
      '(1)Me guía El, con cuánto amor\nMe guía siempre mi Señor;\nAl ver mi esfuerzo en serle fiel,\nCon cuánto amor me guía El.',
      '(2)En el abismo del dolor,\nO en donde brille el sol mejor,\nEn dulce paz o en lucha cruel,\nCon gran bondad me guía El.',
      '(3)Tu mano quiero yo tomar\nJesús, y nunca vacilar,\nPues sólo a quien te sigue fiel\nSe oyó decir: me guía El.',
      '(4)Y mi carrera al terminar,\nY así mi triunfo al realizar,\nNo habrá ni dudas ni temor,\nPues me guiará mi Buen Pastor.',
    ],
  },
  {
    id: '74',
    title: '74 - SUENEN DULCES HIMNOS',
    chorus:
      '¡Gloria! ¡Gloria sea a nuestro Dios!\n¡Gloria, si, cantemos a una voz!\nY el cantar de gloria que se oyó en Belén,\nSea nuestro cántico también.',
    stanzas: [
      '(1)¡Suenen dulces himnos, gratos al Señor,\nY óiganse en concierto universal!\nDesde el alto cielo baja el Salvador\nPara beneficio del mortal.',
      '(2)Montes y collados fluyan leche y miel\nY abundancia esparzan y solaz;\nGócense los pueblos, gócese Israel\nQue a la tierra viene ya la paz.',
      '(3)Salte de alegría, lleno el corazón,\nLa abatida y pobre humanidad;\nDios se compadece viendo su aflicción.\nY le muestra buena voluntad.',
      '(4)Lata en nuestro pecho noble gratitud\nHacia quien nos brinda redención;\nY a Jesús el Cristo que nos da salud.\nTributemos nuestra adoración.',
    ],
  },
  {
    id: '75',
    title: '75 - TU DEJASTE TU TRONO',
    chorus:
      'Ven a mí corazón, ¡oh Cristo!\nPues en él hay lugar para ti;\nVen a mí corazón, ¡oh Cristo! Ven,\nPues en él hay lugar para ti.',
    stanzas: [
      '(1)Tú dejaste tu trono y corona por mí,\nAl venir a Belén a nacer;\nMas a ti no fue dado el entrar al mesón\nY en establo te hicieron nacer.',
      '(2)Alabanzas celestes los ángeles dan,\nEn que rinden al Verbo loor;\nMas humilde viniste a la tierra, Señor,\nA dar vida al más vil pecador.',
      '(3)Siempre pueden las zorras sus cuevas tener,\nY las aves sus nidos también;\nMas el Hijo del Hombre no tuvo un lugar\nEn el cual reclinara su sien.',
      '(4)Tú viniste, Señor, con tu gran bendición\nPara dar libertad y salud,\nMas con odio y desprecio te hicieron morir,\nAunque vieron tu amor y virtud.',
      '(5)Alabanzas sublimes los cielos darán,\nCuando vengas glorioso de allí,\nY tu voz entre nubes dirá: "Ven a mí,\nQue hay lugar junto a mí para ti."',
    ],
  },
  {
    id: '80',
    title: '80 - LA SEÑAL QUE FUE',
    chorus:
      'Ese nombre que era oculto,\nQue no era revelado;\nJesucristo será su nombre\nPor toda la eternidad.',
    stanzas: [
      '(1)La señal que fue desde el principio,\nDada a la humanidad;\nPríncipe de paz será su nombre,\nPor toda la eternidad.',
      '(2)Fue el Mesías grande y verdadero.\nQue a la tierra descendió;\nTrajo a las gentes luz y vida,\nMas el mundo no creyó.',
      '(3)Hay un solo nombre dado al mundo,\nPor el cual se salvará,\nEse nombre da virtud y gracia,\nY el que cree, salvo será.',
      '(4)¡Oh, amigo! ¿Sabes tú que Cristo\nTe podrá salvar a ti?\nVen a El y naciendo de nuevo,\nTe dará su espíritu.',
    ],
  },
  {
    id: '93',
    title: '93 - HUBO QUIEN POR MIS CULPAS',
    chorus:
      'Mis pecados llevó en la cruz, do murió\nEl sublime, el tierno Jesús.\nLos desprecios sufrió, y mi alma salvó\nEl cambió mis tinieblas en luz.',
    stanzas: [
      '(1)Hubo quien por mis culpas\nMuriera en la cruz\nAun indigno y vil como soy:\nSoy feliz, pues su sangre, vertió mi Jesús,\nY con ellas mis culpas borró.',
      '(2)El es tierno y amante, cual nadie lo fue,\nPues convierte al infiel corazón;\nY por esa paciencia, y ternura yo sé,\nQue soy libre de condenación.',
      '(3)Es mi anhelo constante a Cristo seguir,\nMi camino su ejemplo marcó,\nY por darme la vida el quiso morir;\nEn su cruz mi pecado clavó.',
    ],
  },
  {
    id: '99',
    title: '99 - ROSTRO DIVINO',
    chorus: null,
    stanzas: [
      '(1)Rostro divino, ensangrentado,\nCuerpo llagado por nuestro bien;\nCalma benigno justos enojos,\nLloren los ojos que así te ven.',
      '(2)Manos preciosas, tan lastimadas,\nPor mí clavadas en una cruz;\nEn este valle sean mi guía,\nY mi alegría, mi norte y luz.',
      '(3)Bello costado en cuya herida\nHalla su vida la humanidad,\nFuente amorosa de un Dios clemente,\nVoz elocuente de caridad.',
      '(4)Tus pies heridos, Cristo paciente,\nYo indiferente los taladré;\nY arrepentido, hoy que te adoro,\nTu gracia imploro, Señor, peque.',
      '(5)Crucificado en un madero,\nManso Cordero, mueres por mí;\nPor eso el alma triste, llorosa.\nSuspira ansiosa, Señor, por ti.',
    ],
  },
  {
    id: '104',
    title: '104 - NI ORO NI PLATA',
    chorus:
      'Me redimió, más no con plata.\nMe compro el salvador.\nCon oro no, más con su sangre.\nGrande precio de su amor.',
    stanzas: [
      '(1)Ni oro ni plata me han redimido\nRiquezas terrenas no pueden salvar,\nLa sangre de Cristo ya fue mi rescate\nTan sólo por ella el me pudo librar',
      '(2)Ni oro ni plata me han redimido,\nPara ello mi culpa grandísima fue.\nLa sangre de Cristo ya fue mi rescate.\nTan sólo por ella perdón yo hallé.',
      '(3)Ni oro ni plata me han redimido,\nEstando alejado en pecado atroz.\nLa sangre de Cristo ya fue mi rescate\n¡Amor tan sublime y precioso de Dios!',
      '(4)Ni oro ni plata me han redimido,\nDel miedo que nunca me pude quitar,\nLa sangre de Cristo ya fue mi rescate\nTan sólo por ella me pudo librar.',
      '(5)Ni oro ni plata me han redimido,\nEntrada a los cielos jamás pueden dar.\nLa sangre de Cristo ya fue mi rescate\nTan sólo por ella el me pudo comprar.',
    ],
  },
  {
    id: '108',
    title: '108 - HACIA EL SEPULCRO CAMINAN',
    chorus:
      '¿Por qué buscáis al que resucitó?\nAquí no está, la tumba ya dejó.\nId a decir a los que tanto amó:\nJesucristo a ala muerte cruel venció.',
    stanzas: [
      '(1)Hacia el sepulcro caminan con afán\nUnas mujeres piadosas a buscar\nEl cuerpo de su maestro, que murió,\nVan con amor a embalsamar al que partió.',
      '(2)La Magdalena llorando se acercó\nAl jardinero del huerto y preguntó:\n¿A dónde fue conducido mi señor?\nYo quiero ir a disfrutar su gran amor.',
      '(3)Ángeles mil en los cielos cantan hoy\nAl salvador de los hombres, que venció.\nYa no hay sepulcro que pueda retener\nAl que creyó en el Señor, que al fin triunfó.',
    ],
  },
  {
    id: '111',
    title: '111 - MI JESUS ASCENDIO',
    chorus:
      'Cristo ascendió para reinar,\nCon los ángeles está,\nIntercediendo por nosotros desde allí;\nHa de venir aquí a levantar su grey,\nPor que nosotros somos de El.',
    stanzas: [
      '(1)Mi Jesús ascendió a morar\nA la patria que El fue a preparar.\nY sentado a la diestra está\nDel padre por la eternidad.',
      '(2)Tú eres la fuente de amor;\nPrometiste que estando Tú allí,\nEnviarías al consolador\nA morar con nosotros aquí.',
      '(3)Tú hiciste a los cojos andar,\nA los sordos hiciste oír,\nY los mudos pudieron hablar,\nDiste vista a los ciegos aquí.',
      '(4)Hoy te damos las gracias a Ti\nPor la grande manifestación;\nPues tú eres quien sanas aquí;\nY el que das a las almas perdón.',
    ],
  },
  {
    id: '115',
    title: '115 - DIA DE VICTORIA',
    chorus:
      'En las nubes El vendrá,\nEn aquel día final;\nCristo El salvador muy pronto volverá,\nPor aquellos que El amó.',
    stanzas: [
      '(1)Día de victoria y gozo celestial,\nCuando Cristo volverá;\n¡Que glorioso encuentro con mi salvador\nEn las nubes se verá!',
      '(2)Día de gran gozo, día sin igual,\nCuando Cristo volverá.\nDe la tierra al cielo El nos llevará.\nA su seno paternal.',
      '(3)Oye la trompeta que anunciando está,\nLa venida del Señor\nYa no más dolores, ya no más afán,\n¡Con Jesús triunfó el amor!',
    ],
  },
  {
    id: '119',
    title: '119 - VIENE OTRA VEZ',
    chorus:
      '¡Viene otra vez, viene otra vez,\nEn gloria viene al mundo otra vez;\nViene otra vez, viene otra vez\nEl viene pronto a reinar!',
    stanzas: [
      '(1)Cantan los ángeles con dulce voz,\nCanten los hombres con sonora voz;\nCristo vendrá nuestro Rey vencedor,\nCristo vendrá otra vez.',
      '(2)Ved en la tierra, los aires y el mar,\nGrandes señales cumpliéndose ya,\nTodo indicando que pronto vendrá\nNuestro glorioso Señor.',
      '(3)Todos los muertos en Cristo saldrán\nDe sus sepulcros, y alegres irán,\nPara encontrar a su Rey vencedor;\nCristo vendrá otra vez.',
      '(4)¡Ven en las nubes, oh, buen Salvador!\n¡Ven a la tierra, glorioso Señor!\n¡Ven, que tu iglesia te espera, Jesús!\nCristo vendrá otra vez.',
    ],
  },
  {
    id: '125',
    title: '125 - CRISTO EL VIENE',
    chorus:
      'Cristo, El viene, mi amado precioso Señor,\nViene en su gloria, en gloria de gran resplandor\nPor las edades veremos su faz celestial,\nOh, bella historia, cantar de su gloria,\nSomos rescatados del mal.',
    stanzas: [
      '(1)Cristo, El viene, decid la historia,\nPor tierra, desierto y mar.\nCristo, El viene, por sus redimidos,\nQue son rescatados del mal:\n¡Oh, cuanto gozo habrá en el cielo\nPor toda la eternidad!\nCuando estemos con Cristo en Gloria,\nCon El reinaremos allá.',
      '(2)Vuestras cabezas, alzad hoy los santos,\nVuestra redención cerca está;\nLa lucha pasa, la noche se aleja,\nEl día cercano está,\nSi hemos triunfado aquí en el mundo,\nCorona nos espera ya,\nEl viene pronto a quitar los dolores\nY a guiarnos por sendas de paz.',
      '(3)Ven Jesucristo según tú promesa,\nVen pronto oremos así;\nHemos sufrido bastante en el mundo,\nTan sólo esperamos en ti\nGloria a mi Cristo, mi Rey y mi amigo.\nEl viene muy pronto a reinar,\nOh bella historia cantar de su gloria,\nSomos rescatados del mal.',
    ],
  },
  {
    id: '126',
    title: '126 - OH PROCLAMAD CON ALEGRIA',
    chorus:
      '¡Oh! anuncia la historia,\nEl Salvador en plena gloria,\nEn triunfo y victoria,\nAl mundo pronto volverá.',
    stanzas: [
      '(1)¡Oh! proclamad con alegría.\nEl viene en gloria, El viene en gloria;\nA media noche o ya de día\nAl mundo pronto volverá.',
      '(2)No desfallezcas, no desmayes\nEl viene en gloria,\nAsoma luz del sol que raya\nQue pronto aparecerá.',
      '(3)Orad con fe y esperanza,\nEl viene en gloria,\nSeñor, tu pueblo no descansa,\nEn vela te aguardará.',
      '(4)Mirad, que fiel a su promesa,\nEl viene en gloria;\nNo sea te deje en gran sorpresa\nVeloz, Jesús regresará.',
      '(5)¡Oh! dad al rey la bienvenida,\nEl viene en gloria;\nEspérale con luz prendida,\nJesús en gloria reinará.',
    ],
  },
  {
    id: '128',
    title: '128 - ALEGRATE ALMA FELIZ',
    chorus:
      'A mí supremo Rey, alegre cantaré,\nMis ojos han de ver la playa celestial;\nFeliz y salvo soy, y caminando voy\nCon júbilo a mi eterno hogar.',
    stanzas: [
      '(1)Anhelo en las regias mansiones morar,\nDo reina mi Salvador,\nEscucho los ecos de un dulce cantar\nDe triunfo y de gran loor.',
      '(2)Por senda escarpada quizá habré de andar,\nEl mundo me olvidará,\nMás en las riberas del límpido mar\nLos santos me esperan ya.',
      '(3)Gloriosa esperanza, inefable la paz\nQue siento en mi corazón;\n¡Cuán dulce es tener comunión y solaz\nCon Dios en adoración!',
      '(4)Eleva tu vista y contempla a Jesús,\nSe fiel a tu Rey y Señor;\nLa luz de la faz de mi buen Salvador\nTe envuelvan en su esplendor.',
    ],
  },
  {
    id: '133',
    title: '133 - DIOS NOS HA DADO PROMESAS',
    chorus:
      'Lluvias de gracia,\nLluvias pedimos, Señor,\nMándanos lluvias copiosas,\nLluvias del consolador.',
    stanzas: [
      '(1)Dios nos ha dado promesas:\nLluvias de gracia enviaré,\nDones que os den fortaleza;\nGran bendición os daré.',
      '(2)Cristo nos dio la promesa\nDel santo consolador,\nDándonos paz y pureza,\nPara su gloria y honor.',
      '(3)¡Oh! Dios, a todo creyente\n¡Muestra tu amor y poder!\nTú eres de gracia la fuente,\nLlena de paz nuestro ser.',
      '(4)Obra en tus siervos piadosos\nCelo virtud y valor\nDándonos dones preciosos,\nDones del consolador.',
    ],
  },
  {
    id: '149',
    title: '149 - SOY PEREGRINO AQUI',
    chorus:
      'Este mensaje fiel oíd,\nQue dijo ya celeste voz:\n"Reconciliaos ya" dice el Señor y Rey\n¡Reconciliaos hoy con Dios!',
    stanzas: [
      '(1)Soy peregrino aquí, mi hogar lejano está\nEn la mansión de luz eterna paz y amor:\nEmbajador yo soy del reino celestial\nEn los negocios de mi rey.',
      '(2)Que del pecado vil arrepentidos ya,\nHan de reinar con El los que obedientes son:\nEs el mensaje fiel que debo proclamar,\nEn los negocios de mi Rey.',
      '(3)Mi hogar más bello es que el valle de Sarón.\nEterno gozo y paz reinan por siempre en él.\nY allí Jesús dará eterna habitación;\nEs el mensaje de mi Rey.',
    ],
  },
  {
    id: '211',
    title: '211 - ¡TODO A CRISTO!',
    chorus:
      'Yo me rindo a Él, yo me rindo a Él;\nTodo a Cristo yo me entrego,\nQuiero serle fiel.',
    stanzas: [
      '(1) Todo a Cristo yo me rindo,\nCon el fin de serle fiel;\nPara siempre quiero amarle,\nY agradarle sólo a Él.',
      '(2) Todo a Cristo yo me rindo,\nA sus pies postrado estoy;\nLos placeres he dejado\nY le sigo desde hoy.',
      '(3) Todo a Cristo yo me rindo,\nSí, de todo corazón;\nYo le entrego alma y cuerpo,\nBusco hoy su santa unción.',
      '(4) Todo a Cristo he rendido,\nSiento el fuego de su amor;\n¡Oh, qué gozo hay en mi alma!\n¡Gloria, gloria a mi Señor!',
    ],
  },
  {
    id: '213',
    title: '213 - ¡MÁS DE JESÚS!',
    chorus:
      'Más quiero amarle,\nMás quiero honrarle,\nMás de su salvación gozar,\nMás de su dulce amor gustar.',
    stanzas: [
      '(1) Más de Jesús deseo saber,\nMás de su gracia y poder,\nMás de su salvación gozar,\nMás de su dulce amor gustar.',
      '(2) Más quiero a Jesús seguir,\nMás de su santa ley cumplir,\nMás de su voluntad saber,\nMás de su espíritu tener.',
      '(3) Más de Jesús, más oración,\nMás cerca estar en comunión,\nMás su palabra meditar,\nMás sus promesas alcanzar.',
      '(4) Más de Jesús allá veré,\nMás semejante a Él seré,\nMás de su gloria he de gozar,\nMás su gran nombre alabar.',
    ],
  },
  {
    id: '214',
    title: '214 - ¡QUE MI VIDA ENTERA ESTÉ!',
    chorus:
      'Lávame en tu sangre, Salvador,\nLímpiame de toda mi maldad;\nTraigo a ti mi vida, para ser, Señor,\n¡Tuya por la eternidad!',
    stanzas: [
      '(1) Que mi vida entera esté,\nConsagrada a ti Señor;\nQue a mis manos pueda guiar\nEl impulso de tu amor.',
      '(2) Que mis pies tan sólo en pos\nDe lo santo puedan ir,\nY que a ti Señor, mi voz,\nSe complazca en bendecir.',
      '(3) Que mi tiempo todo esté\nConsagrado a tu loor,\nQue mis labios al hablar\nHablen sólo de tu amor.',
      '(4) Toma ¡Oh, Dios! Mi voluntad,\nY hazla tuya nada más;\nToma, sí mi corazón,\nPor tu trono lo tendrás.',
      '(5) Toma tú mi amor que hoy\nA tus pies vengo a poner;\nToma todo lo que soy,\nSeñor, ¡Todo tuyo quiero ser!',
    ],
  },
  {
    id: '215',
    title: '215 - ¿VIVIRÁS POR CRISTO?',
    chorus:
      'Su gran poder sus dones te dará,\nEl gozo y paz a tu alma volverá;\nDel corazón las manchas limpiará,\nQue haga en ti Su santa voluntad.',
    stanzas: [
      '(1) ¿Vivirás por Cristo, fiel a Él al fin serás?\nEn la vía de paz, en pos de Él andarás;\n¿De la cruz al pie tus cuitas todas dejarás?\nQue haga en ti Su voluntad.',
      '(2) ¿Quieres ir en pos de Cristo y llevar la cruz?\n¿Quieres conocer la paz que da Jesús?\n¿Quieres que te guíe con la verdadera luz?\nQue haga en ti Su voluntad.',
      '(3) ¿Quieres en el cielo tus amigos encontrar?\nDebes preparar tu vida sin tardar;\n¿Quieres a Él sólo hoy tu vida consagrar?\nQue haga en ti Su voluntad.',
    ],
  },
  {
    id: '218',
    title: '218 - ¡PUEDO OÍR TU VOZ!',
    chorus:
      'Seguiré donde Tú me guíes,\nSi tu mano me conduce,\nSalvador, seguirte quiero,\nDondequiera fiel te seguiré.',
    stanzas: [
      '(1) Puedo oír tu voz llamando\nCon acentos cariñosos;\nPuedo oír tu llamamiento:\nTrae tu cruz y ven en pos de mí.',
      '(2) Yo te seguiré en el huerto,\nPor la senda dolorosa;\nY al seguirte siempre alegre\nSufriré contigo mi Jesús.',
      '(3) Sufriré por ti, Maestro,\nSi tu gracia me sostiene;\nY si Tú irás conmigo,\nMoriré contigo, mi Jesús.',
      '(4) Me darás la gracia y gloria,\nY me sostendrás por siempre;\nCada hora yo te quiero,\nY por siempre Tú me guiarás.',
    ],
  },
  {
    id: '236',
    title: '236 - ¡SALVADOR MI BIEN ETERNO!',
    chorus: null,
    stanzas: [
      '(1) Salvador, mi bien eterno,\nMás que vida para mí;\nEn mi fatigosa senda\nCerca siempre te halle a Ti;\n::Junto a Ti, junto a Ti::\nEn mi fatigosa senda\nCerca siempre te halle a Ti;',
      '(2) No los bienes, no placeres,\nNi renombre busco aquí,\nEn las pruebas, en desdenes,\nCerca siempre te halle a Ti.\n::Junto a Ti, junto a Ti::\nEn las pruebas, en desdenes,\nCerca siempre te halle a Ti;',
      '(3) Yendo por sombrío valle,\nEn rugiente mar hostil,\nAntes y después del trance,\nCerca siempre te halle a Ti.\n::Junto a Ti, junto a Ti::\nAntes y después del trance,\nCerca siempre te halle a Ti;',
    ],
  },
  {
    id: '242',
    title: '242 - JUNTO A MI DIOS',
    chorus: null,
    stanzas: [
      '(1)Junto a mi Dios deseo siempre morar,\nAunque jamás el mundo me quiera amar.\nQue del mundo no soy, que del mundo no soy\nSólo por Él tránsito, al cielo voy.',
      '(2)Sólo a mi Dios el alma sabe querer,\nPorque de amor Él llena todo mi ser,\nVeraz es mi Señor, veraz es mi Señor.\nQue dice en su palabra: "Dios es amor".',
      '(3)¡Mi Dios, mi bien, mi todo! Llégate a mí;\nNo permitas que more lejos de Ti.\nTú me recibirás; Tú me recibirás,\nQue estar sin Ti no puedo nunca, jamás.',
    ],
  },
  {
    id: '261',
    title: '261 - ¡CUANDO TE QUIERO MAS!',
    chorus:
      ':: Cuando te quiero más::,\nCerca Tú estás, mi buen Salvador,\nCuando te quiero más.',
    stanzas: [
      '(1)Cuando te quiero cerca Tú estás;\nMi buen Jesús, no me dejarás;\nSiempre amoroso me sostendrás,\nCuando te quiero más:',
      '(2)Cuando te quiero listo Tú estás,\nY abandonarme nunca podrás;\nPaz por tristeza siempre me das,\nCuando te quiero más.',
      '(3)Cuando te quiero no faltarás,\nHora tras hora Tú me guiarás;\nTiernos cuidados Tú me darás,\nCuando te quiero más.',
      '(4)Cuando te quiero vienes Jesús\nY tu presencia muy dulce es,\nCorren las aguas de Salvación,\nCuando te quiero más.',
    ],
  },
  {
    id: '264',
    title: '264 - ¡POR JESUS FUI HALLADO!',
    chorus:
      'En Jesús mi Señor sea mi gloria eterna,\nEl me amó y me salvó en su gracia tierna',
    stanzas: [
      '(1)Lejos de mi padre Dios, por Jesús fui hallado;\nPor su gracia y por su amor, sólo fui salvado',
      '(2)En Jesús, mi salvador, pongo mi confianza;\nToda mi necesidad suple en abundancia.',
      '(3)Cerca de mi buen Pastor vivo cada día;\nToda gracia en mi Señor halla el alma mía.',
      '(4)¡Guárdame, Señor Jesús, para que no caiga!\nComo un sarmiento en la vid, vida de ti traiga',
    ],
  },
  {
    id: '269',
    title: '269 - ¡COMPRADO CON SANGRE!',
    chorus:
      'Lo sé Lo se comprado con sangre yo soy,\nLo sé Lo se con Cristo al cielo yo voy.',
    stanzas: [
      '(1)Comprado con sangre por Cristo. (4)Seguro sé que la belleza,\nCon gozo al cielo yo voy; Del gran Rey yo voy a mirar,\nLibrado por gracia infinita, Ahora me guarda y me guía,\nYa sé que su hijo yo soy Y siempre me quiere ayudar',
      '(2)Soy libre de pena y culpa,\nSu gozo el me hace sentir;\nEl llena de gracia mi alma,\nCon el es tan dulce vivir',
      '(3)En Cristo yo siempre medito,\nY nunca le puedo olvidar,\nCallar sus favores no quiero,\nVoy siempre a Jesús alabar',
    ],
  },
  {
    id: '272',
    title: '272 - ¡MAESTRO SE ENCRESPAN LAS AGUAS!',
    chorus:
      'Los vientos, las ondas oirán tu voz:\n"¡Sea la paz!" "¡Sea la paz!"\nCalmas las iras del negro mar,\nLas luchas del alma las haces cesar,\nY así la barquilla do va el Señor,\nHundirse no puede en el mar traidor,\nDoquier se cumple tu voluntad,\n¡Sea la paz! ¡Sea la paz!\nTu voz resuena en la inmensidad:\n"¡Sea la paz!"',
    stanzas: [
      '(1)Maestro, se encrespan las aguas\nY ruge la tempestad;\nLos grandes abismos del cielo\nSe cubren de obscuridad.\n¿No ves que aquí perecemos?\n¿Puedes dormir así,\nCuando el mar agitado nos abre\nProfundo sepulcro aquí?',
      '(2)Maestro, mi ser angustiado\nTe busca con ansiedad.\nDe mi alma en los antros profundos\nSe libra cruel tempestad;\nPasa el pecado a torrentes\nSobre mi frágil ser,\nY perezco, perezco, Maestro,\n¡Oh quiéreme socorrer!',
      '(3)Maestro, paso la tormenta,\nLos vientos no rugen ya,\nY sobre el cristal de las aguas\nEl sol resplandecerá,\n¡Maestro, prolonga está calma,\nNo me abandones más;\nCruzaré los abismos contigo\nGozando bendita paz!',
    ],
  },
  {
    id: '282',
    title: '282 - ¡CUANDO COMBATIDO!',
    chorus:
      'Cuenta las riquezas de tu Dios;\nMira las riquezas de su amor.\nPon los ojos donde Cristo está,\nY tu mente guarde la divina paz.',
    stanzas: [
      '(1)Cuando combatido por la adversidad,\nCreas ya perdida tu felicidad,\nMira lo que el cielo para tí guardó,\nCuenta las riquezas que el Señor te dio.',
      '(2)¿Andas agobiado por algún pesar?\n¿Tu cruz te parece dura de llevar?\nCuenta las promesas del Señor Jesús,\nY de las tinieblas nacerá la luz.',
      '(3)Cuando de otros veas la prosperidad,\nY tus pies claudiquen tras de su maldad,\nCuenta las riquezas que tendrás por fe,\nDonde el oro es polvo que hollará tu pie.',
      '(4)Aunque grande sea tu aflicción aquí,\nNo te desalientes: Dios está por ti.\nSi a su propio hijo no se reservó,\nAún verás riquezas que ojo nunca vio.',
    ],
  },
  {
    id: '315',
    title: '315 - ¡NO TE DE TEMOR!',
    chorus:
      'No te dé temor, no te dé temor,\nNunca, nunca, nunca;\nEs tu amado Salvador\nNunca, pues, te dé temor',
    stanzas: [
      '(1)¡No te dé temor hablar por Cristo!\nHaz que brille en ti su luz;\nAl que te salvó confiesa siempre,\nTodo debes a Jesús.',
      '(2)¡No te dé temor hacer por Cristo\ncuanto de tu parte está!\nObra con amor, con fe y constancia:\nTus trabajos premiará.',
      '(3)¡No te dé temor sufrir por Cristo\nLos reproches o el dolor!\nSufre con amor tus pruebas todas,\nCual sufrió tu Salvador',
      '(4)¡No te dé temor vivir por Cristo\nEsa vida que te da!\nSi tan sólo en El por siempre fiares,\nEl con bien te sacará',
      '(5)¡No te dé temor morir por Cristo!\nVía, verdad, y vida es El;\nEl te llevará con su ternura,\nA su célico vergel',
    ],
  },
  {
    id: '348',
    title: '348 - ¡MIRAD ENFERMOS AL SEÑOR!',
    chorus: null,
    stanzas: [
      '(1)Mirad enfermos al señor; El sanará sí, sanará\nSu sangre en cruenta cruz vertió. El sanará, sí, sanará.\nSegún tu fe así será, Jesús jamás nos faltara;\nEl es el mismo hoy que ayer, y por los siglos ha de ser.',
      '(2)Pedid con fe: Jesús hará, El sanará, sí sanará,\nEl es la fuente de salud: El sanará, sí, sanará,\nY las señales seguirán y los enfermos sanarán.\nTestigos son de su virtud, que es eternal, si eternal.',
      '(3)Confía pues en tu Señor, El sanará, si, sanará.\nEntrega a El tu débil ser; El sanará, si sanará.\nA miles ha sanado ya; a muchos más El sanará,\nY tú de estos puedes ser, El sanará, si sanará.',
    ],
  },
  {
    id: '364',
    title: '364 - ¡USA MI VIDA!',
    chorus:
      'Usa mi vida, usa mi vida,\nPara tu gloria, oh Jesús;\nTodos los días y hoy quiero ser,\nTestigo tuyo, Señor, por doquier',
    stanzas: [
      '(1)Muchos que viven en tu derredor\nTristen, hambrientos están\nTú, por tu vida, les puedes llevar\nGozo, luz y bendición.',
      '(2)Di a los tristes que Dios es amor,\nEl quiere dar su perdón,\nA los que vienen a Cristo Jesús\nBuscando paz, salvación',
      '(3)Toda tu vida hoy rinde al Señor;\nCada momento se fiel,\nOtros que vean en ti su amor\nPronto se rindan a El.',
    ],
  },
  {
    id: '370',
    title: '370 - ¡LA MERCED DE NUESTRO PADRE!',
    chorus:
      '¡Mantened el Faro ardiendo!\n¡Arrojad su luz al mar!\nQue si hay nautas pereciendo\nLes podréis así salvar',
    stanzas: [
      '(1)La merced de nuestro Padre\nEs un faro en su brillar,\nEl nos cuida y nos protege\nCon las luces de alta mar.',
      '(2)Reina noche de pecado,\nRuge airado negro mar,\nAlmas hay que van buscando\nEsas luces de alta mar.',
      '(3)Ten tu lámpara encendida,\nQue en la tempestad habrá\nAlgún náufrago perdido,\nY tu luz le salvará',
    ],
  },
  {
    id: '389',
    title: '389 - ¡CUANDO LA TROMPETA SUENE!',
    chorus: '::Cuando allá se pase lista::\nA mi nombre yo feliz responderé',
    stanzas: [
      '(1)Cuando la trompeta suene\nEn aquel día final.\nY que el alba eterna rompa en claridad:\nCuando las naciones salvas\nA su patria lleguen ya,\nY que sea pasada lista, allí he de estar.',
      '(2)En aquel día sin nieblas,\nEn que muerte ya no habrá,\nY su gloria el Salvador impartirá;\nCuando los llamados entren\nA su celestial hogar,\nY que sea pasada lista, allí he de estar.',
      '(3)Trabajemos por el Maestro,\nDesde el alba al vislumbrar;\nSiempre hablemos de su amor y fiel bondad;\nCuando todo aquí fenezca.\nY nuestra obra cese ya,\nY que sea pasada lista allí he de estar.',
    ],
  },
  {
    id: '392',
    title: '392 - ¡NO HABRA SOMBRAS!',
    chorus:
      'Sombras, nada de sombras\nAl dejar el mundo de dolor;\nSombras, nada de sombras\nCuando al cielo llegue el vencedor.',
    stanzas: [
      '(1)No habrá sombras en el valle de la muerte,\nCuando cese de la vida el batallar,\nY escuchemos del Señor el llamamiento,\nYa llevándonos con El a descansar.',
      '(2)Al dejarnos los que amamos no habrá sombras.\nSi su fe depositaron en Jesús;\nPorque irán para vivir por las edades\nCon quien quiso redimirnos en la cruz.',
      '(3)Cuando venga por los suyos no habrá sombras\nPues su gloria y majestad las destruirán;\nY las huestes redimidas con su jefe,\nA las célicas mansiones entrarán.',
    ],
  },
  {
    id: '409',
    title: '409 - ¡DA LO MEJOR AL MAESTRO!',
    chorus:
      '¡Da lo mejor al Maestro! A combatir por El ven\nSé paladín decidido de la verdad y del bien.',
    stanzas: [
      '(1)¡Da lo mejor al Maestro! tu fuerza, tu juventud,\nTodo el fervor de tu alma, tu exhuberante salud.\nDueño sea El de tu vida; sírvele con fe y amor.\nTodo a su gracia lo debes. ¡Dale a Jesús lo mejor!',
      '(2)¡Da lo mejor al Maestro! Entrégale el corazón.\nNadie como el te ha amado; digo es de tu devoción.\nRico y feliz quiere hacerte; es gran tesoro su amor,\nDeja que llene tu alma. ¡Dále a Jesús lo mejor!',
      '(3)¡Da lo mejor al Maestro! Has por amor suyo el bien.\nPara serviros El vino; sirve tú a otros también.\nDi a los perdidos, que El salva, que El cura todo dolor.\nQue ama, consuela y ayuda, ¡Dale a Jesús lo mejor!',
    ],
  },
  {
    id: '426',
    title: '426 - ¡SOY REDIMIDO!',
    chorus:
      'Soy redimido esto es muy cierto\nY un Cristo vivo llevo en mi alma\nSi estoy caído El está presto\nA levantarme con su poder.',
    stanzas: [
      '(1)En negro fango yo estaba hundido\nY allí perdido, Jesús me halló\nY con su brazo fuerte extendido\nDe aquel peligro me rescató.',
      '(2)Si estás caído como yo estaba\nY tu alma vaga en el error\nCristo es tu amigo dale tu carga\nQue sólo El salva y da perdón.',
      '(3)Pronto estaremos los redimidos\nAllí reunidos con El Señor\nDe gracia llenos siendo testigos\nQue por los siglos es Dios de amor.',
    ],
  },
  {
    id: '433',
    title: '433 - ¡OH QUE AMIGO!',
    chorus: null,
    stanzas: [
      '(1)¡Oh, qué amigo nos es Cristo!\nEl llevó nuestro dolor,\nY nos manda que llevemos\nTodo a Dios en oración.\n¿Vive el hombre desprovisto\nDe paz, gozo y santo amor?\nEsto es porque no llevamos\nTodo a Dios en oración.',
      '(2)¿Vives débil y cargado\nDe cuidados y temor?\nA Jesús, refugio eterno,\nDile todo en oración.\n¿Te desprecian tus amigos?\nCuéntaselo en oración,\nEn sus brazos amorosos\nPaz tendrá tu corazón.',
      '(3)Jesucristo es nuestro amigo,\nDe esto pruebas El nos dió\nAl sufrir el cruel castigo\nQue el culpable mereció;\nY su pueblo redimido\nHallará seguridad,\nFiando en este Amigo eterno\nY esperando en su bondad.',
    ],
  },
  {
    id: '434',
    title: '434 - ¡NADIE PUDO AMARME COMO CRISTO!',
    chorus:
      'Nadie pudo amarme como Cristo,\nEs incomparable su amistad;\nSólo El pudo redimirme del pecado,\nPor su amor y su bondad.',
    stanzas: [
      '(1)Yo quisiera hablarte del amor Cristo,\nPues en el hallé un amigo fuerte y fiel,\nPor su gracia transformó mi vida entera,\nLo que en esta vida soy lo debo a El.',
      '(2)Mi alma estaba llena de ayes y tristezas,\nLlena estaba de miserias y dolor;\nCon ternura Cristo me tendió la mano.\nY me guió por el sendero del amor.',
      '(3)Cada día viene darme nuevo aliento,\nA mi corazón infunde dulce paz;\nNo comprenderé por que vino a salvarme,\nHasta que en el cielo pueda ver su faz.',
    ],
  },
  {
    id: '436',
    title: '436 - ¡HALLE UN BUEN AMIGO!',
    chorus: null,
    stanzas: [
      '(1)Hallé un buen amigo, mi amado salvador.\nContaré lo que El ha hecho para mí;\nHallándome perdido e indigno pecador.\nMe salvó y hoy me guarda para sí.\nMe salva del pecado, me guarda de Satán.\nPromete estar conmigo, hasta el fin; (Aleluya)\nEl consuelo mi tristeza, me quita todo afán;\n¡Grandes cosas Cristo ha hecho para mí!',
      '(2)Jesús jamás me falta, jamás me dejará.\nEs mi fuerte y poderoso Protector.\nDel mundo me separo, y de la vanidad,\nPara consagrar mi vida al señor.\nSi el mundo me persigue, si sufro tentación.\nConfiando en Cristo puedo resistir; (Aleluya)\nLa victoria me es segura, y elevo mi canción:\n¡Grandes cosas Cristo ha hecho para mí!',
      '(3)Yo sé que Jesucristo muy pronto volverá\nY entre tanto me prepara un hogar.\nEn la casa de mi Padre, mansión de luz y paz,\nDo el creyente fiel con El ha de morar;\nLlegándome a la gloria, ningún pesar tendré.\nContemplaré su rostro siempre allí; (Aleluya)\nCon los santos redimidos gozoso cantaré;\n¡Grandes cosas Cristo ha hecho para mí!',
    ],
  },
  {
    id: '437',
    title: '437 - ¡ES JESUS EL MEJOR AMIGO!',
    chorus:
      '::!Es Jesús el mejor amigo!::\nEl tus súplicas oirá, y tu carga llevará;\n¡Es Jesús el mejor amigo!',
    stanzas: [
      '(1)Es Jesús el mejor amigo,\nCuando triste o tentado estés;\nColmará de bendición, tu transido corazón;\n¡Es Jesús el mejor amigo!',
      '(2)En Jesús fiel amigo encuentro,\nPaz perfecta El a mi alma da; Apoyado\nEn El estoy, sin temor mi ser le doy,\nEs Jesús el mejor amigo',
      '(3)Aunque ande en algún peligro,\nO en sombra de muerte esté; Ningún mal\nMe alcanzará, pues Jesús me amparará,\nEs Jesús el mejor amigo',
      '(4)Cuando estemos al fin reunidos\nCon los redimidos más allá; cantaremos\nCon fervor en presencia del Señor,\nEs Jesús el mejor amigo',
    ],
  },
  {
    id: '438',
    title: '438 - ¡CRISTO DEL CIELO!',
    chorus:
      '¡Soy salvo, este es mi canto!\nCristo me salva ¡Oh cuán glorioso!\n¡Soy salvo, miro su gloria!\nDoy alabanzas al Salvador',
    stanzas: [
      '(1)Cristo del cielo a buscarme,\nVino a la tierra, se humilló:\nCuando vagaba yo en la noche,\nEl me buscó y mi alma salvó.',
      '(2)Cristo me guarda del pecado,\nEl es mi guía a su mansión;\nGrandes riquezas de su gracia\nEl derramó en mi corazón.',
      '(3)El me conduce por la senda;\nMe libra siempre de tropezar;\nEl es mi apoyo y es mi fuerza,\nY si le sigo no puedo flaquear.',
      '(4)Cristo muy pronto, con voz tierna,\nHa de llamarme al dulce hogar,\nDo voy a verle, en su gloria.\nSu triunfo siempre allí celebrar.',
    ],
  },
  {
    id: '446',
    title: '446 - ¡ES JESUCRISTO MI TODO!',
    chorus:
      'Cristo, Cristo, Tú eres mi Salvador,\nCristo, Cristo, tuyo seré, Señor,\nTe seguiré por doquiera si tú guiando vas\nY al terminar mi carrera, en gloria veré tu faz.',
    stanzas: [
      '(1)Es Jesucristo mi todo, grato es cantar su loor,\n¡Oh cuán sublime e infinito es su divino amor!\nCuando me vió errabundo, cual hijo pródigo,\nVino a buscar y a salvarme, y a su redil me llevó.',
      '(2)Cristo es el lirio del valle, la rosa es de Sarón.\nCristo es el astro esplendente, la rosa de salvación.\nEl es la fuente de vida, y gozo eternal,\nYa satisface mi alma con el maná celestial.',
      '(3)Cristo nació en un pesebre, la amarga copa bebió.\nCual inocente cordero, en el calvario murió;\nResucitó de la tumba, y al cielo ascendió.\nMás pronto viene en gloria, esa promesa nos dió.',
    ],
  },
  {
    id: '447',
    title: '447 - ¡JESUS ES MI REY SOBERANO!',
    chorus: null,
    stanzas: [
      '(1)Jesús es mi Rey soberano,\nMi gozo es cantar su loor;\nEs Rey y me ve cual hermano,\nEs Rey y me imparte su amor.\nDejando su trono de gloria,\nMe vino a sacar de la escoria.\n¡Y yo soy feliz, y yo soy feliz con El!',
      '(2)Jesús es mi amigo anhelado.\nY en sombras o en luz siempre va,\nPaciente y humilde a mi lado,\nSu ayuda y consuelo me da:\nPor eso constante lo sigo,\nPorque el es mi Rey y mi amigo,\n¡Y yo soy feliz, y yo soy feliz con El!',
      '(3)Señor ¿que pudiera yo darte,\nPor tanta bondad para mi?\n¿Me basta servirte y amarte?\n¿Es todo entregarme yo a ti?\nEntonces, acepta mi vida,\nQue a Ti sólo queda rendida. ¡Pues yo\nSoy feliz, pues yo soy feliz por Ti!',
    ],
  },
  {
    id: '475',
    title: '475 - ¡DIOS BENDIGA A LAS ALMAS UNIDAS!',
    chorus: null,
    stanzas: [
      '(1)Dios bendiga a las almas unidas\nPor los lazos de amor sacrosanto,\nY las guarde de todo quebranto\nEn el mundo de espinas erial.\nQue el hogar que a formarse comienza\nCon la unión de estos dos corazones.\nGoce siempre de mil bendiciones.\nAl amparo del Dios de Israel',
      '(2)Que el Señor con su dulce presencia,\nCariñoso estas bodas presida,\nY conduzca por sendas de vida\nA los que hoy han jurado lealtad.\nLes recuerde que nada en el mundo\nEs eterno que todo termina,\nY por tanto, con gracia divina,\nCifrar deben la dicha en su Dios.',
      '(3)Que los dos que al altar se aproximan\nA jurarse su fe mutuamente,\nBusque siempre se Dios en la fuente\nEl secreto de dicha inmortal.\nY si acaso de duelo y tristeza\nSe empañasen sus sendas un día\nEn Jesús hallarán dulce guía,\nQue otra senda les muestre mejor.',
    ],
  },
  {
    id: '483',
    title: '483 - ¡POR LA INEFABLE!',
    chorus: null,
    stanzas: [
      '(1)Por la inefable luz del día,\nQue tanto alegra al corazón,\nAl despertarnos diariamente,\nCuando admiramos el fulgor\nDe una alborada rubia y bella,\n¡Gracias sean dadas a mi Dios!',
      '(2)Por el aroma de las flores,\nEn cuyas galas de color\nVemos la mano omnipotente,\nCuyo pincel las matizó:\nPor tantas, tantas maravillas\n¡Gracias sean dadas a mi Dios!',
      '(3)Por ese pan de cada día,\nQue El nos concede con amor,\nInfatigable, bueno y santo,\nA todo ser de la creación.\nPor ese pan, que nos da vida\n¡Gracias sean dadas a mi Dios!',
      '(4)Y por el techo que nos cubre,\nY por su santa bendición,\nY por la vida de mis padres.\nQue me arrullaron con amor;\nPor tantas dádivas preciosas\n¡Gracias sean dadas a mi Dios!',
      '(5)Por esa chispa milagrosa\nDe inteligencia, que El nos dió,\nPara pensar en lo infinito\nCon alas bellas de amor,\nPor ese excelso privilegio,\n¡Gracias sean dadas a mi Dios!',
      '(6)Por esa dicha tan insigne,\nQue en su piedad nos dispensó;\nMandando a su Hijo Sacrosanto\nA consumar la Redención\nDe los humanos pecadores.\n¡Gracias sean dadas a mi Dios!',
    ],
  },
  {
    id: '505',
    title: '505 - CRISTO EL MEDICO',
    chorus:
      'Culpas y pecados El quitó,\nPor su sangre que en la cruz vertió\nAlma que en pecado atada estás,\nVen a Cristo, y libertad tendrás.',
    stanzas: [
      '(1)Cristo el médico del alma es,\nSabe bien la prueba en que te ves.\nLuchas que has tenido en oración\nTodo sabe bien tu Salvador.',
      '(2)Ola embravecida El calmará,\nAlma hambrienta Cristo saciará.\nEn Jesús, el pobre pecador\nHallará un grande Salvador',
      '(3)Pecador que buscas salvación,\nCristo el Médico te da perdón.\nCon su diestra siempre te guiará.\nY a los cielos El te llevará.',
    ],
  },
  {
    id: '515',
    title: '515 - ¡OH PADRE BENDITO!',
    chorus:
      'En tu plan divino yo estaba contado\nDe ser redimido, por siempre salvado.\nTan sólo por gracia, porque no era digno,\nYa que en el pecado yo estaba perdido',
    stanzas: [
      '(1)¡Oh Padre bendito cuán grande es tu amor,\nMis negros delitos tu sangre borró.\nEl día que contrito imploré tu perdón,\nSentí que fui limpio de mi corazón',
      '(2)Desde que he nacido en tu santa fe.\nUn cambio he sentido en todo mí ser.\nEn mí se ha cumplido tu palabra fiel,\nPues he comprendido, que he vuelto a nacer.',
      '(3)Tú has prometido muy pronto volver,\nLlevarme contigo al paraíso aquel,\nDo Tú por los siglos serás nuestro Rey,\nCon tus escogidos también yo estaré.',
    ],
  },
  {
    id: '519',
    title: '519 - ¡CONFIO EN DIOS!',
    chorus:
      'Confío en Dios, yo sé que El ve por mí,\nEn alta cumbre o en rugiente mar.\nMi alma en El segura está,\nMi Padre siempre velará por mí.',
    stanzas: [
      '(1)Confío en Dios doquiera que yo esté,\nEn tierra o mar en calma o tempestad.\nDe día en día, cuanto venga a mí,\nMi Padre siempre velará por mí.',
      '(2)La rosa es un objeto de su amor,\nEl guía el ave, que en su vuelo va,\nSeguro estoy que El piensa en mí;\nMi Padre siempre velará por mí.',
      '(3)Confío en Dios, aún entre fieras mil,\nEn la prisión o en duro batallar,\nEn gloria, fama o fuego vil,\nMi Padre siempre velará por mí.',
      '(4)En valle obscuro o en profundidad,\nEl buen Pastor su oveja guiará,\nCon gran bondad hacía el hogar;\nMi Padre siempre velará por mí.',
    ],
  },
  {
    id: '526',
    title: '526 - ¡MAS ALLA DEL SOL!',
    chorus:
      ':: Más allá del sol:: Yo tengo un hogar,\nHogar, bello hogar, más allá del sol',
    stanzas: [
      '(1)Aunque en esta vida no tenga riquezas\nSé que allá en la gloria tengo mi mansión.\nCual alma perdida entre la pobreza,\nDe mí, Jesucristo tuvo compasión.',
      '(2)Así por el mundo yo voy caminando.\nDe pruebas rodeado y de tentación.\nPero Jesucristo, que me está probando\nMe llevará salvo hasta su mansión.',
      '(3)A todas las razas del linaje humano,\nCristo quiere darles plena salvación.\nTambién una casa, no hecha de manos,\nFue a prepararnos a la santa Sión.',
    ],
  },
];
