...
export const htmlTemplate = (
    ...
    /** In-WebView "Copiar sección" / "Copiar todo" — only for Strong's modal, not notes/print */
    strongCopyUi: boolean = false
) => {
    const rawDefinition =
        content?.[0]?.definition || "No hay resultado para esta palabra";
    const normalizedDefinition = normalizeDictionaryDefinitionHtml(rawDefinition);
    const definitionBody =
        strongCopyUi && !isPrint
            ? wrapStrongSectionsForCopy(normalizedDefinition)
    ...
    return `
        ...
        <head>
            ...
            <style>
                ...
                ${strongCopyUi && !isPrint
            ? `
                /* Strong cards: borders-only depth, calm reading rhythm (no heavy shadow + border stack) */
                .strong-def-sections {
                    display: flex;
        ...
        <div class="selectable-definition">${definitionBody}</div>
            ${strongCopyUi && !isPrint
            ? `<script>
(function () {
