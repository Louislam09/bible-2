...
const getPreviewText = (noteText: string | undefined): string => {
    ...
    const cleaned = noteText.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
    ...
    const notesHtml = notes.map((note, index) => `
        ...
        >
            <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                ...
                <svg class="w-5 h-5" style="color: ${subtleText};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-sm truncate mt-0.5" style="color: ${mutedText};">
                    ${getPreviewText(note.note_text)}
                </p>
            </div>
            
            <div class="flex flex-col items-end flex-shrink-0">
                <span class="text-xs" style="color: ${mutedText};">
                    ${formatDate(note.updated_at || note.created_at)}
                </span>
                <svg class="w-4 h-4 mt-1" style="color: ${mutedText};" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    const emptyStateHtml = `
        <div class="flex flex-col items-center justify-center py-20 px-8 text-center">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center mb-5"
                 style="background: ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)'};">
                <svg class="w-8 h-8" style="color: ${mutedText};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                </svg>
            </div>
            <p class="text-theme-text font-medium text-base mb-1">Sin notas</p>
            <p class="text-sm" style="color: ${mutedText};">Toca + para crear una</p>
        </div>
    `;
...
<body class="m-0 p-0 text-theme-text bg-theme-background select-none overflow-y-auto scrollbar-hide min-h-screen">
    ...
    <!-- Header -->
    <div class="sticky top-0 bg-theme-background z-10 px-4 pt-4 pb-3 flex items-center justify-between">
        <h1 class="text-xl font-semibold text-theme-text">Notas</h1>
        <div class="flex items-center gap-3">
            <span class="text-sm" style="color: ${mutedText};">${notes.length}</span>
            <button
                class="create-row w-9 h-9 rounded-full flex items-center justify-center"
