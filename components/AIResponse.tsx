...
export default function AIResponse({ response }: AIResponseProps) {
    const theme = useTheme(); const [copySuccess, setCopySuccess] = React.useState(false);
        ...
        <View style={styles.container}>
            <View style={[styles.toolbar, { backgroundColor: theme.colors.card }]}>
                <TouchableOpacity
                    ...
                    onPress={handleCopy}
                >
                    <Icon name={copySuccess ? "Check" : "Copy"} size={20} color={copySuccess ? theme.colors.notification : theme.colors.text} />
                    <Text style={[
                        styles.toolbarButtonText,
                        { color: copySuccess ? theme.colors.notification : theme.colors.text }
                    ]}>{copySuccess ? 'Copiado' : 'Copiar'}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.toolbarButton, { borderColor: theme.colors.border }]}
                    onPress={generatePDF}
                >
                    ...
                    <Text style={[styles.toolbarButtonText, { color: theme.colors.text }]}>Compartir PDF</Text>
                </TouchableOpacity>
            </View>
            <ScrollView
