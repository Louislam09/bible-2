import { Image, StyleSheet, Dimensions, ScrollView, ActivityIndicator } from 'react-native'
...
const EventCard = ({ event, period, index = 0, styles  }: { event: TimelineEvent, period: TimelinePeriod, index?: number, styles: ReturnType<typeof getStyles> }) => {
  return (
    <Animated.View 
      entering={FadeIn
        .duration(800)
    ...
    >
      ...
      <View style={styles.dateContainer}>
        <Text style={styles.date}>{Math.abs(event.start)} {event.start < 0 ? 'A.C.' : 'D.C.'}</Text>
        {event.end !== event.start && (
          <>
            <Text style={styles.date}> - </Text>
            <Text style={styles.date}>{Math.abs(event.end)} {event.end < 0 ? 'A.C.' : 'D.C.'}</Text>
          </>
        )}
      </View>
      <Text style={styles.description} numberOfLines={2}>{period.description}</Text>
    </Animated.View>
  )
}

const CarouseItem = ({ imageURI, index, scrollX, activeColor, styles }: { activeColor: string, imageURI: string; index: number, scrollX: SharedValue<number>, styles: ReturnType<typeof getStyles> }) => {
...
const CircularSlider = () => {
  ...
  const onScroll = useAnimatedScrollHandler((e) => {
    ...
    const newActiveIndex = clamp(e.contentOffset.x / _itemTotalSize, 0, eventImages.length - 1)
  ...
  return (
    <View style={{ flex: 1,backgroundColor: theme.colors.background }}>
      ...
      {currentEvents?.length > 0 && currentPeriod && (
        <>
          <View style={[styles.counterContainer, { backgroundColor: theme.colors.background + 99 }]}>
            <View style={styles.periodHeader}>
              <Text style={styles.sectionTitle}>{currentPeriod.sectionTitle}</Text>
          ...
          <View style={{ zIndex: 2, backgroundColor: theme.colors.background + 99 }}>
            <ScrollView 
              style={{ 
      ...
      <Animated.FlatList
        style={{ flexGrow: 0, height: _itemSize * 1.6, marginBottom: _spacing, zIndex: 13, position: 'absolute', bottom: 0 }}
        contentContainerStyle={{
          gap: _spacing,
