...
const CurrentWordModal: React.FC<Props> = ({ strongNumber, setOpen }) => {
    ...
    return (
        <View style={{ height: '50%', width: '100%', backgroundColor: '#607D8B', position: 'absolute', bottom: 0 }}>
            <View style={styles.chapterHeader}>
                <View style={{ flex: 1, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-evenly' }}>
                    {nav.canGoBack && <TouchableOpacity style={{ flex: .1 }} onPress={handleGoBack}>
                        <Ionicons name="arrow-back" size={30} color="white" />
                    </TouchableOpacity>}
                    {nav.canGoForward && <TouchableOpacity style={{ flex: .1 }} onPress={handleGoForward}>
                    ...
                    <Text style={styles.chapterHeaderTitle}>
                        ...
                        }}>{strongNumber.text}</Text>
                    </Text>
                </View>
                <TouchableOpacity style={{ flex: .1, paddingHorizontal: 10 }} onPress={() => setOpen()}>
                    <Ionicons name="close" size={30} color="white" />
                </TouchableOpacity>
            </View>
            {data?.definition && <WebView
                ref={webViewRef}
                originWhitelist={['*']}
                source={{ html: htmlTemplate(data ?? 'There is not data for this word.') }}
